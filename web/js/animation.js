...
export function springMotion({
    ...
    return new Promise(resolve => {
        const frame = now => {
            ...
            if (onUpdate) onUpdate(position, now - start);
            else object.position.copy(position);

            const elapsed = now - start;
            const settled =
                elapsed >= duration ||
                (position.distanceTo(to) < 0.008 && velocity.length() < 0.02);
            ...
            if (onUpdate) onUpdate(to, elapsed);
            else object.position.copy(to);
            resolve();
        };
        requestAnimationFrame(frame);
    });
...
export async function enterCube(renderer) {
    ...
    await springMotion({
        ...
        onUpdate: (position, elapsed) => {
            ...
            // Slightly lagged rotational settling, synchronized with the fall.
            const eased = t * t * (3 - 2 * t);
            renderer.cubeRoot.quaternion.slerpQuaternions(startQ, targetQ, eased);
        },
    });
...
export async function exitCube(renderer) {
    ...
    const h = renderer.container.clientHeight;
    const end = new THREE.Vector3(0, -Math.max(5.0, 3.8 + h / 260), 0);
    ...
    return new Promise(resolve => {
        const frame = now => {
            ...
            const e = t * t;
            renderer.cubeRoot.position.lerpVectors(start, end, e);

            const q = new THREE.Quaternion();
            q.setFromAxisAngle(new THREE.Vector3(0.7, 0.25, 0.55).normalize(), 0.9 * t);
            renderer.cubeRoot.quaternion.copy(startQ).premultiply(q);
